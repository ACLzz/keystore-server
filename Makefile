...
create_database:
    ...
    psql -h $(DB_HOST) -p $(DB_PORT) -U $$DB_USER -d postgres -v db="$(DB_NAME)" -v username="$(NEW_DB_USER)" -v dev=$$isDev -f database-setup.sql

setup: build move_config create_database

define umyml
	$(shell grep $1 $(CONFIG) | tail -n1 | awk '{print $$2}')
endef